...
public class ThirdPersonShooterController : MonoBehaviour
{
    ...
    private void SetThirdPersonShooter()
    {
        ...
        if (starterAssetsInputs.shoot)
        {
            if (player.CurrentAmmo > 0 && player.CurrentAmmo <= player.MaxCurrentAmmo)
            {
                    ...
                    && !GameObject.Find("Player").GetComponentInChildren<AbilityHolder>().performingAbility)
                {
                    ...
                    */
                    ...
                    GameObject.Find("GameManager").GetComponent<GameManager>().PlaySoundEffect(gunAudio, shoot, 0.3f, 0.8f);
                    firing.StartFiring();
                    //player.AddExp(10);
                    //player.AddGold(500);
            ...
            else
            {
                ...
                GameObject.Find("GameManager").GetComponent<GameManager>().PlaySoundEffect(gunAudio, noAmmo, 0.3f, 1f);
            }
    ...
    private void Reload()
    {
        if (player.CurrentAmmo < player.MaxCurrentAmmo && player.MaxAmmo > 0) //To make sure that you have ammo to reload
        {
            ...
            GameObject.Find("GameManager").GetComponent<GameManager>().PlaySoundEffect(gunAudio, reload, 0.3f, 1f);
